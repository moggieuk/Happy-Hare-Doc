...
| `Feature-Fan-Control.md` | `Kconfig.fans`, generated fan hardware/parameters, and the first-class `MMU_FAN` command | new — the one real gap the original macro audit found | **done (item 62), updated for managed-fan rework** — documents shared and per-gate managed fans, environment/MCU temperature sources, generated `mmu_hardware.cfg` and `mmu_parameters.cfg` settings, runtime targeting, and the distinction from fixed Klipper heater fans. The obsolete `_MMU_FAN_VARS` macro workflow and its manual fan/sensor pairing workaround are removed. Real menuconfig screenshots come from the `feature-fan-control` session. |
...
| `Reference-Macro-Vars.md` | `config/base/mmu_macro_vars.cfg` (the real shipped template) + `installer/macro_vars/Kconfig.*` — not the wiki | **done** — same role for `mmu_macro_vars.cfg` that `Reference-Parameters.md` plays for generated parameters. Organized by macro block in the template's own order, with Blobifier's large block split into its source groupings. Fan control is no longer a macro-variable block; its generated defaults live in `Reference-Parameters.md` and its runtime interface on `Feature-Fan-Control.md` / `Reference-Commands.md`. |
...
The auto-generated `T[[i]]` per-gate tool macros deliberately have no page
here: they have no variables of their own and are already covered as part of
`Slicer-Setup.md`'s T0 discussion.
