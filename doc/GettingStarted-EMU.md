...
To populate the per-gate MCU options, you should now exit the installer using
`Q` or `Esc`, confirm `Y`es to save, and re-run the installer as `./install.sh
-i`.
...
the EBB42/36 defaults you picked earlier. The one you're most likely to need to
...
| Nowmal bowden unload speed   | 250 mm/s |
| Short movement speed         | 80 mm/s  |
| Shorte movement acceleration | 80 mm/s² |
...
`````{.text .console-output} MMU_SENSORS DETAIL=1 mmu_entry_0           -->
TRIGGERED mmu_entry_1           --> Open filament_compression  --> Open
...
```{.text .console-output} MMU_SYNC_FEEDBACK Sync feedback: Neutral ```
...
```{.text .console-output} MMU_LED_EFFECT EFFECT=gate_status GATE=0
...
```{.text .console-output} MMU_CALIBRATE_GEAR MEASURED=102.5 ```
...
-------------------------------------------- | -------- |
---------------------------- | | `PARAM_SYNC_FEEDBACK_ANALOG_MAX_COMPRESSION` |
`0.9435` | Max compression analog value | |
`PARAM_SYNC_FEEDBACK_ANALOG_MAX_TENSION`     | `0.0982` | Max tension analog
value     | | `PARAM_SYNC_FEEDBACK_ANALOG_NEUTRAL_POINT`   | `0.5275` | Neutral
point                |
...
```{.text .console-output} MMU_SELECT GATE=0 MMU_LOAD MMU_UNLOAD ```
...
now than mid-print; see [Operation: Debugging
...
instead of a wipe tower: either [Macro: Purge](Macro-Purge.md) (simple, prints a
purge line) or [Macro: Blobifier](Macro-Blobifier.md) (a dedicated purge/park
station, more capable but its own hardware). See [Purging without a wipe
tower](Feature-Tip-Forming-Purging.md#purging-without-a-wipe-tower) for how to
disable the slicer's tower and hand purging over to Happy Hare.
